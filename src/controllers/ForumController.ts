...
class ForumController {
    ...
    if (user) {
      ...
      if (post.length > 0) {
        try {
          ...
          //send notification
          const get_users = await Schema.User()
            .find({ department: dept, level: level })
            .where({ pushToken: { $exists: true } });
  ...
  static async getPosts(request: Request, response: Response) {
    ...
    if (user) {
      try {
        const posts = await Schema.Post()
          .find({ level: user.level, department: user.department })
          .sort({ _id: -1 });
  ...
  static async addComment(request: Request, response: Response) {
    ...
    if (post) {
      ...
      if (comment.length > 0) {
        ...
        return response
          .status(404)
          .send({ message: "Comment can not be empty " });
      }
    } else {
  ...
  static async getComments(request: Request, response: Response) {
    ...
    try {
      const comments = await Schema.Comment()
        .find({ post: pid })
        .sort({ _id: -1 });
      //find best anser
      const best = await Schema.Comment()
        .findOne({ post: pid })
        .sort({ likes: -1 });
      response.status(200).send({
        comments,
  ...
  static async likeComment(request: Request, response: Response) {
    ...
    if (user) {
      try {
        await Schema.Comment().updateOne(
          {
            _id: cid,
          },
          {
  ...
  static async deleteComment(request: Request, response: Response) {
    ...
    if (user && post && comment) {
      try {
        await Schema.Comment().deleteOne({
          _id: cid,
        });
